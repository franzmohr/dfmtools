...
#ifndef DFMTOOLS_BAYESTS_REPORTER_H
#define DFMTOOLS_BAYESTS_REPORTER_H
...
///
/// The sampler calls `check_interrupt()` and `progress()` once per draw and
/// does no throttling of its own, so both are throttled here:
/// `Rcpp::checkUserInterrupt()` is an R evaluation and a chain of 20000 draws
/// should not pay for 20000 of them. `interrupt_every` matches the
/// every-hundredth-draw check the R implementation this replaced used.
...
#endif // DFMTOOLS_BAYESTS_REPORTER_H
