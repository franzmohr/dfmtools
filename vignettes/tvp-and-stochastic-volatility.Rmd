...
it is not true; the last section says what that mode is and what keeps the chain
out of it.
...
#>      Qtr1 Qtr2 Qtr3 Qtr4
#> 2007 0.22 0.25 0.31 0.37
#> 2008 0.47 0.58 0.72 0.80
...
#> [1] 0.023 0.041 0.023 0.027 0.041 0.029 0.022 0.024 0.033
```
...
Nothing above passed `method` to `add_initial_values()`, and the default it took
is the reason the loadings at the top of this vignette came out positive.

The loadings are the one block whose starting value decides which mode the
sampler ends up in rather than how long it takes to get there. Only the product
of the loadings and the factor is identified, and the restriction that pins it
fixes the leading block of `lambda` rather than anything about the factor, so a
start whose free loadings are negative where the data want them positive is a
coherent model in its own right: the mirror, in which the factor is the negative
of the common component and the series whose loading is fixed at plus one is
treated as noise, its idiosyncratic variance absorbing nearly all of its
variation. It fits far worse, and burn-in does not escape it, since turning the
factor around would have to pass through configurations no single Gibbs step will
take.

`method = "pca"`, the default, starts the loadings at the estimate the leading
principal components of the panel imply, rotated onto the identifying
restriction. That estimate is what the data say and does not depend on the seed.
The alternative draws them, as every other block is drawn:
...
which on this panel is a draw with a standard deviation of ten -- the default
prior precision on a loading is 0.01 -- so its signs are close to a coin toss.
Estimating the four models above that way, over four seeds each, lands six of
sixteen runs in the mirror; from the principal components start, none of them
do.

That removes the cause rather than the possibility: a start on the right side of
the likelihood is not a guarantee on a sample too short to hold the chain there.
The sign of the loadings stays worth a look, as at the top of this vignette, and
a factor negatively correlated with a panel of activity series is the symptom.
